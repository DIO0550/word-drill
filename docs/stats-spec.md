...
ただし、連続学習日数は学習継続の指標として常に全期間で計算する。
...
期間フィルタが有効な集計は `history` を一次データとして再計算し、`stats` は全期間のキャッシュ用途に限定する。
...
| フィールド | ソース | フォーマット | 空の場合 |
|:----------|:-------|:------------|:---------|
| 単語名 | フィルタ期間内の `history.questionId` を問題データに紐付けて `term` を取得 | テキスト | - |
| 正答率 | フィルタ期間内の正解数 / 回答総数 | パーセント | - |
| 回答回数 | フィルタ期間内の正解数 + 不正解数 | `{正答数}/{総回答数}` | - |
